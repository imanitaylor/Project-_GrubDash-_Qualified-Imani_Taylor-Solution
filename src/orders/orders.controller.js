...
function orderIncluesDishes(req, res, next) {
  const {
    data: { dishes },
  } = req.body;
...
function validOrderDishes(req, res, next) {
  const {
    data: { dishes },
  } = req.body;
  for (let i = 0; i < dishes.length; i++) {
    //makes sure that dishes exists, has a number, and is not 0
    if (
      !dishes[i].quantity ||
      typeof dishes[i].quantity !== "number" ||
      dishes[i].quantity < 1
    ) {
      return next({
        status: 400,
